...
def main(args):
    ...
    # Check if the dataset has 'question' and 'answer' fields (like gsm8k)
    # If so, combine them. Otherwise, assume a 'text' field.
    if 'question' in dataset['train'].column_names and 'answer' in dataset['train'].column_names:
        print("Found 'question' and 'answer' columns, combining them into 'text'.")
        ...
        raise ValueError("Dataset does not have a 'text' column, and is not structured like gsm8k. Please check the dataset format.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset-name', type=str, default='roneneldan/TinyStories', help="Name of the Hugging Face dataset to use.")
    parser.add_argument('--dataset-config', type=str, default=None, help="Configuration for the dataset (e.g., 'main' for gsm8k).")
    parser.add_argument('--tokenizer-name', type=str, default='gpt2', help="Name of the tiktoken tokenizer to use (e.g., 'gpt2', 'r50k_base').")
    parser.add_argument('--output-dir', type=str, default='hrm_language/data/TinyStories-tokenized', help="Directory to save the tokenized .bin files and meta.json.")
